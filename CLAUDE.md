...
### 2. Starting a ticket ("start LG-004")
Do these in order and write the results into the ticket file:
...
4. **🧑‍💻 Session plan.** Split the work into *Claude scaffolds* (imports, types, signatures, wiring, config,
   test fixtures) and *Arren writes* (the logic that teaches the concept). Get Arren's OK, then scaffold.
5. **Guided coding.** Arren types; Claude reviews what he wrote, answers questions, and gives **hints before
   answers**. Only give the full solution when he asks for it.
6. **Verify.** `testing` agent adds or runs tests → `security` agent reviews anything touching data, the
   network, auth, prompts, or dependencies → all checks green.
